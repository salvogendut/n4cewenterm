...
QR_QUERY	EQU	0	;query
QR_RESPONSE	EQU	128	;response
;
;  A four bit field that specifies kind of query in this
;  message.  This value is set by the originator of a query
;  and copied into the response.  The values are:
OP_QUERY	EQU	0	;a standard query (QUERY)
OP_IQUERY	EQU	8	;an inverse query (IQUERY)
OP_STATUS	EQU	16	;a server status request (STATUS)
;
AA_FLAG0	EQU	4	;AND-mask for AA-Bit
;
TC_FLAG0	EQU	2	;AND-mask for TC-Bit
;
...
;
RA_FLAG1	EQU	128	;AND-mask for RA-Bit
;
Z_FLAG1		EQU	0	;3 reserved bits set to zero
;
...
GHBAD1:	PUSH	HL
	...
	LD	(HL),750 & 0xFF
	INC	HL
	LD	(HL),750 >> 8	;load first Timeout/2 in ms
	POP	HL
